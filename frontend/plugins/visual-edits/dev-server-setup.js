...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow BigBean domains (add your production domains here)
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*bigbeancafe\.in$/)) {
        return true;
      }
      ...
      try {
        ...
        for (const change of variableEditChanges) {
          ...
          if (result.success) {
            ...
            try {
              execSync(`git -c user.name="visual-edit" -c user.email="support@bigbeancafe.in" add "${result.file}"`);
              execSync(`git -c user.name="visual-edit" -c user.email="support@bigbeancafe.in" commit -m "visual_edit_variable_${timestamp}"`);
            } catch (gitError) {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@bigbeancafe.in" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@bigbeancafe.in" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
