...
import { LayoutDashboard, FolderOpen, Package, ShoppingCart, Plus, Edit, Trash2 } from 'lucide-react';
...
const CategoriesPage = () => {
  ...
  const fetchCategories = async () => {
    try {
      const response = await api.get('/categories?type=bakehouse');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const data = { ...formData, type: 'bakehouse' };
      if (editingId) {
  ...
  return (
    <DashboardLayout navigation={navigation}>
      <div>
        <h1 className="text-3xl font-bold text-coffee-900 mb-6">Bakehouse Categories</h1>
            ...
            <div className="space-y-2">
              {categories.map((cat) => (
                <div key={cat.id} className="flex justify-between items-center p-3 bg-coffee-50 rounded-lg">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => { setFormData({ name: cat.name, description: cat.description || '' }); setEditingId(cat.id); }} className="text-coffee-600 hover:text-coffee-800">
