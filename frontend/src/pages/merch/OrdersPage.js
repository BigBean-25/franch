...
const OrdersPage = () => {
  ...
  const fetchOrders = async () => {
    try {
      const response = await api.get('/orders?order_type=merch');
  ...
  return (
    <DashboardLayout navigation={navigation}>
      <div>
        <h1 className="text-3xl font-bold text-coffee-900 mb-6">Merch Orders</h1>
              ...
              <div key={order.id} className="bg-white rounded-lg shadow p-6 border border-coffee-200">
                ...
                {order.status === 'pending' && (
                  <div className="flex gap-2 mt-4">
                    <button onClick={() => handleApprove(order.id)} className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg">
                      <CheckCircle className="w-5 h-5" />
                      Approve
                    </button>
                    <button onClick={() => handleReject(order.id)} className="flex items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-lg">
